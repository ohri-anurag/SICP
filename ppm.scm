...
; This is a very basic PPM parser library.
; It does NOT correctly parse PPM files that have maxval greater than 255.
...
; Read the PPM from a file
  (define (read-binary-ppm-file file)
    (let ((port (open-file-input-port file)))
      (if (binary-ppm? port)
        (parse-parameters port)
        "Invalid input file.")))

  ; Check if the PPM is a binary PPM file, ie magic number should be P6.
  (define (binary-ppm? binary-port)
    (let ((bv (get-bytevector-n binary-port 3))) ; 3 characters including newline
      (and
        (= (bytevector-u8-ref bv 0) 80)       ; For P
        (= (bytevector-u8-ref bv 1) 54))))    ; For 6
  ...
  ; Parse the binary matrix(not exactly stored as a matrix) of 3*w*h bytes.
  (define (parse-matrix w h maxval port)
    (cons (list w h maxval)
      (split
        (bytevector->u8-list (get-bytevector-n port (* 3 w h)))
        (* 3 w))))

  ; Write the PPM to another file
  ...
  (define (write-binary-ppm-file file ppm)
    (let ((port (open-file-output-port file (file-options no-fail))))
      (put-bytevector port #vu8(80 54 10))           ; Magic Number (P6), followed by a newline
      (write-parameters port (car ppm))
      (put-bytevector port (u8-list->bytevector (concat (cdr ppm))))
      (close-port port)))
  ...
  ; Invert the colors of a ppm, using maxval
  (define (invert ppm)
    (map-matrix
      (lambda (x) (- (caddar ppm) x))
      ppm))

  ; (define (avg items) (floor (/ (accumulate + 0 items) (length items))))
  (define (avg a b) (floor (/ (+ a b) 2)))

  (define (downscale-x bytes)
    (concat 
      (map
        (lambda (pixel-data)
          (let ((two-pixels (split pixel-data 3)))
            (map avg (car two-pixels) (cadr two-pixels))))
        (split bytes 6))))
  ...
  (define (flip-horiz image)
    (cons (car image)
          (map (lambda (row) (concat (reverse (split row 3)))) (cdr image))))

  ; rogers-path points to the PPM file containing the image of W.B. Rogers.
  (define rogers-path "rogers.ppm")

  (define rogers (read-binary-ppm-file rogers-path))
  )